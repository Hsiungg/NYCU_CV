        ...
        with open(annotation_path, 'r') as f:
            ...
            if 'annotations' in data:
                ...
                if isinstance(data['annotations'], dict):
                    print(
                        f"Annotation keys: {list(data['annotations'].keys())[:5]}")
                    first_key = list(data['annotations'].keys())[0]
                    print(
                        f"First annotation entry: {data['annotations'][first_key]}")
...
def process_livecell_dataset(coco_annotation_path, base_image_dir, output_dir, cell_type=None, all_masks=None):
    # Create output directories
    train_img_dir = os.path.join(output_dir, 'images')
    ...
    for img_id in tqdm(img_ids, desc="Processing images"):
        ...
        # Create mask
        mask = np.zeros(
            (img_info['height'], img_info['width']), dtype=np.uint16)
...
def main():
    parser = argparse.ArgumentParser(
        description='Process LiveCell dataset for Cellpose training')
    ...
    output_dir = 'data/livecell_processed'
    ...
    all_masks = process_livecell_dataset(
        train_anno_path, base_image_dir, output_dir, filter_cell_type)

    print("\nProcessing validation set...")
    all_masks = process_livecell_dataset(
        val_anno_path, base_image_dir, output_dir, filter_cell_type, all_masks)
